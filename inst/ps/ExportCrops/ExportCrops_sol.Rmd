...
#< ignore
```{r "setup"}
# Load the libraries that are necessary for RTutor to work.
library(RTutor)

# Adapt the working directory, then run setup chunk in RStudio.
setwd("C:/Users/Adrian Sihler/OneDrive/Desktop/MA_Publish/Masterarbeit Projekt/RTutor Problem Set")

# Set the name of the Problem Set and the corresponding solution file.
ps.name = "ExportCrops"; sol.file = paste0(ps.name,"_sol.Rmd")

# Character vector of all packages that are mandatory to load for the Problem Set.
libs = c("haven", "dplyr", "modelsummary", "kableExtra", "fixest", "ggplot2", "ggpubr", "conleyreg", "tidyr", "foreign")

# Creation of the Problem Set.
create.ps(sol.file = sol.file, ps.name = ps.name, libs = libs, addons = "quiz", user.name = NULL, stop.when.finished = FALSE)
          
# Show the Problem Set in the Browser.
...
#< quiz "Banana Crop Origin"

question: Since the focus of the study **"Export Crops and Civil Conflict"** relies on the horticultural banana crop, where do you think most bananas come from?
sc:
    - Central and Southern Europe
    - North, Middle and Southern America
    - Latin America, East India, and Southeast Asia*
    - Middle East and Western Asia
success: Great, you are correct! Sources from DevelopmentAid (2023) show that most of the high-value banana crops come from developing countries in Latin America, East India, and Southeast Asia.

failure: Try again. Google can be a powerful tool to apply.

#>

#< info "Banana production worldwide"

According to DevelopmentAid (2023), bananas are a major global agricultural export commodity. The industry is valued at USD25 billion and projected to grow at 4.5% annually from 2022 to 2027. The banana production rose by about 25% from 109 to 135 million metric tons between 2010 and 2022, as shown in the following Figure 2: 

![](Image_2_banana_production.png)  
<br/>

Additionally, the top ten banana cultivating countries worldwide can be seen in Figure 3:

![](Image_3_banana_producers.png) 
#>
...
#< quiz "Location of the Philippines"

question: Given that the Philippines consists of several island groups, what do you think is the number of regions and provinces in the country?
sc:
    - 17 regions with 81 provinces*
    - 18 regions with 92 provinces
    - 20 regions with 86 provinces
    - 15 regions with 105 provinces
success: Great, you are correct! The territory is separated into 17 different regions, consisting of 81 total provinces, as shown in the following Figure 5.

failure: Try again. You can look on a map and try to count.
...
#< quiz "Poverty Regions of the Philippines"

question: Given the previous Figure 7 about the regional poverty differences, which regions are the poorest among the Philippines?
sc:
    - BARMM, Visayas, National Capital Region 
    - BARMM, Caraga, Visayas*
    - National Capital Region, Cordillera Administrative Region, Calabarzon
    - Bicol Region, Soccsksargen, Mindanao
success: Great, you are correct! The previous graphic illustrated that the regions with the highest poverty rates were BARMM, Caraga, Visayas, Soccsksargen and Mindanao, which coincided with the main banana-producing regions for the export market.

failure: Try again. Take a closer look.
...
```{r "1.1.1", fig.width = 10, fig.height = 6}
#< task_notest
...
#< quiz "Banana Global Market Price"

question: Given the previously created graph about the global banana price, what was the peak banana price level that was ever recorded?
sc:
    - USD1.357 per kg 
    - USD1.833 per kg
    - USD1.685 per kg*
    - USD1.464 per kg
success: Great, you are correct! The highest ever price level was recorded in December 2022 and January 2023, reaching USD1,685 per metric ton of bananas, equivalent to USD1.685 per kg.

failure: Try again. Take a closer look.

#>

#< award "Banana Republic Award"

**Excellent, you have done a great job!** This award recognizes your new knowledge about the banana industry and its global impact. You have learned economic facts about the culinary banana crop and discovered some interesting current information about the Philippines, one of the major banana producers and `e`xporters in the world. *You have shown that you love bananas and that you are a citizen of the world!*

You can proudly display this award in your trophy cabinet by using the command `awards()` in a code chunk and sharing it with your friends.
...
#< quiz "Terrorism in the Philippines"

question: Considering these armed rebel groups, what do you intuitively expect, how the Philippines compare with other countries, in terms of the impact of terrorism on its security, as of the year 2020 when our research article was published? The lower the rank, the greater the impact of terrorism.
sc:
    - Rank 10 of the 195 countries in the world*
    - Rank 115 of the 195 countries in the world
success: Great, you are correct! It is astonishing that such a beautiful and economically prosperous country as the Philippines was ranked 10th in the world in terms of countries affected by terrorism in 2020, according to the Global Terrorism Index 2020 from the Institute for Economics and Peace (2020, p. 8).

failure: Try again. You can use Google but keep in mind that we look at the year 2020!
...
#< quiz "Extortion in the Philippines"

question: What is your guess of the total extortion revenues that the NPA obtains, just from the banana plantation region of Mindanao alone?
sc:
    - USD17 million
    - USD32 million
    - USD28 million*
    - USD41 million
success: Great, you are correct! According to Crost and Felter (2020, p. 1492), the government provides an estimate of the magnitude of the extortion revenues accumulated by the NPA from the agricultural export sector in the Mindanao region. They report that the NPA extorts more than USD28 million annually from the owners of banana plantations and processing facilities. The NPA resorts to violent attacks and property destruction if the owners refuse to pay.
...
#< quiz "Framework of Conflict 1"

question: How do you expect the level of violence to vary, if, in the author's model, the banana plantation is situated in a region where the government has a dominant position?
sc:
    - Low level of violence*
    - High level of violence
success: Great, you are correct! It is not that intuitive to think about. As stated by Crost and Felter (2020, p. 1494), the intensity of violence will be lower if the plantation is situated in a region where the government has a substantial advantage over the NPA in terms of strength The insurgents will not have the resources to contest the governmental authority.

failure: Unfortunately, you messed up the fifty-fifty-chance, sorry.

#>

#< quiz "Framework of Conflict 2"

question: And now the other way around, how do you expect the level of violence to vary, if, in the author's model, the banana plantation is situated in a region where the NPA insurgent group has a dominant position?
sc:
    - Low level of violence*
    - High level of violence
success: Great, you are correct! Similar as before, stated by Crost and Felter (2020, p. 1494) as well, the intensity of violence will be lower if the plantation is situated in a region where the insurgent group has a substantial advantage over the governmental forces in terms of capacity. Government authorities will lack the resources to challenge the NPA dominance, allowing it to easily establish a local monopoly.

failure: Unfortunately, you messed up the fifty-fifty-chance, sorry.

#>

#< quiz "Framework of Conflict 3"

question: In addition, there exists another potential scenario. How would the degree of violence fluctuate, if, in the author's model, the banana plantation is in a region where the governmental forces and the NPA insurgent group have comparable power?
sc:
    - Low level of violence
    - High level of violence*
success: Great, you are correct! The intensity of violence will be higher if the plantation is situated in a region where the governmental forces and the insurgent group have comparable power. Crost and Felter (2020, p. 1494) explain this phenomenon from the conflict framework as follows. The intense conflict between the parties is driven by their shared belief in the likelihood of victory, leading them to resort to violent tactics to expel their rivals from the territory. Therefore, it is the initial level of provincial rebel capacity that determines the impact of a rise in insurgent operations.
...
#< quiz "Rebel Force and Violence"

question: Considering the information presented, to check if you have understood the mechanism correctly, what factors do you believe contribute to an increase in provincial violence? 
sc:
    - Increase of the rebel capacity from a high starting point or a worsen of the insurgent strength from a low baseline
    - Decline of the rebel capacity from a high starting point or an improvement of the insurgent strength from a low baseline*
    - Decline of the rebel capacity from a low starting point or an improvement of the insurgent strength from a high baseline
success: Great, you are correct! Conversely, an increase in violence may indicate either a decline of the rebel capacity from a high starting point or an improvement of the insurgent strength from a low baseline regional impact (moving towards the local maximum of the inverted parabola). The local maximum represents the point where the government and the rebels have an equal impact on the region, as shown by Crost and Felter (2020, p. 1494f.) in Figure 10, with a high intensity of conflict.

failure: Try again! Examine the graph more carefully or review the previous quizzes once again.

#>

#< award "Enlightener Medal"

**Congratulations!** You have conducted extensive research on the topic and mastered the essential knowledge about the key `i`nsurgent groups in the Philippines, their strategies, and the core model of the extortion mechanism with its impact on violence. *You are ready for the future challenges that await you!*  
...
To read in specific files with an `.dta` format, we necessarily need the `read_dta()` function from the `haven` package. 

Therefore, the first thing we do is to load the necessary `haven` package. For all packages, this is always done with the help of the `library()` function. Inside the parentheses, we need to pass the name of the required package with quotation marks being set.
...
Afterwards, we need to use the `read_dta` function that is now available. As a parameter, we supply it with the file name and the working directory path, where you saved your `.dta` file. It is mandatory that this parameter be read in quotation marks as well. If we assume that you want to read in the file `dta_file_for_R.dta`, which is saved in your current working directory, the executed code should be as follows: 

```{r "2.1.2", eval= FALSE}

read_dta("dta_file_for_R.dta")

```

Now you have understood the concept of reading in a data set, but you have not assigned it to a variable yet. *As a result, you will not be able to refer to the data set at any time during a specific exercise*. To solve this problem, we need to assign the command to a variable like `data_variable`. This is done as follows:

```{r "2.1.3", eval= FALSE}

data_variable <- read_dta("dta_file_for_R.dta")

#Note that if you want to declare new variables in R, it does not matter if you write
#variable <- function(...) or
#variable = function(...). 
#The edge can only be used for variable declaration, the equal sign has several other uses, whereby it should not be confused with!

```

This is the entire process of reading a data set and having a reference to it at any time. *When you face similar tasks as this Problem Set, this should always be your first step from which you can proceed.*

It should be noted that documentation is available for almost all R packages and the functions that belong to them. This [site](https://www.rdocumentation.org/) can be extremely useful for finding packages and corresponding functions!
...
library("haven")
#< hint
display("In case you don't remember how to load a package, just take a quick look at the Info block above!")
#>
...
#< hint
display("In case you don't remember how to read in a data frame, just take a quick look at the Info block above! Remember the quotation marks around the file name!")
#>
...
These are the 62 variables of our data set that need explanation. The following list is sorted according to the order in which it appears, as you can see from the `head()` command above:

* `year`: Year of observation

* `typhoon`: Dummy variable, whether the province was hit by a major typhoon or not (mean)

* `wettemp`: Temperature during the wet season (mean) *(from the rasterized world data frame of the Tropical Rainfall Measuring Mission, normalized according to their scale)*

* `temperature`: Temperature during the dry season (mean) *(from the rasterized world data frame of the Tropical Rainfall Measuring Mission, normalized according to their scale)*

* `mindanao`: Dummy variable, whether the observed province is Mindanao or not

* `latcoord`: Latitude coordinates of the observed province

* `longcoord`: Longitude coordinates of the observed province

* `pop2000`: Province population in 2000

* `mus_vinc`: Violent incidents involving the MILF

* `ctm_vinc`: Violent incidents involving the NPA

* `asg_vinc`: Violent incidents involving the ASG

* `le_vinc`: Violent incidents involving the LE (Lawless Elements: armed group)

* `govtvinc`: Incidents with at least one government casualty

* `envinc`: Incidents with at least one insurgent casualty

* `civvinc`: Incidents with at least one civilian casualty

* `vinc`: Recorded violent incidents

* `totcas`: Recorded casualties

* `eninitvinc`: Violent incidents from a dispute initiated by insurgents

* `govinitvinc`: Violent incidents from a dispute initiated by the government

* `zrain`: Annual rainfall index *(from the rasterized world data frame of the Tropical Rainfall Measuring Mission, normalized according to their scale)*

* `wetrain`: Wet season rainfall index *(from the rasterized world data frame of the Tropical Rainfall Measuring Mission, normalized according to their scale)*

* `meancavendish`: Cavendish banana production (mean)

* `sugarprice`: Average annual sugar price in 2010 USD

* `bananaprice`: Average annual Cavendish banana price in 2010 USD

* `lacatanprice_farmgate`: Domestic average annual Lacatan banana price *(NOT in 2010USD)*

* `sabaprice_farmgate`: Domestic average annual Saba banana price *(NOT in 2010USD)*

* `riceprice`: Average annual rice price in 2010 USD

* `usd_php_xrate`: Exchange rate 2010 USD - Philippine Peso

* `pcode`: Unique numeric code for each province

* `visayas`: Dummy variable, whether the observed province is Visayas or not  

* `cavendish2002_pop`: Cavendish banana production (kg per capita)

* `logcavendishinter2002_pop`: Cavendish banana production (kg per capita) times Log price

* `logpinter_pop`: Cavendish banana production times rainfall in Ecuador

* `noinfilXlogcavinter02_pop`: Cavendish banana production times Log price times low insurgent NPA control

* `infil_pcntg0Xlogcavinter02_pop`: Cavendish banana production times Log price times medium insurgent NPA control

* `infil_pcntg50Xlogcavinter02_pop`: Cavendish banana production times Log price times high insurgent NPA control

* `sugar2002_pop`: Sugar production (kg per capita)

* `lacatan2002_pop`: Lacatan banana production (kg per capita) 

* `saba2002_pop`: Saba banana production (kg per capita)

* `rice2002_pop`: Rice production (kg per capita) 

* `F_logcavendishinter2002_pop`: Cavendish banana production times Lead of Log price (Log price in t+1)

* `logcavendishinter_std`: Cavendish banana production standard deviation times Log Cavendish price

* `logsabaownpriceinter_std`: Saba banana production standard deviation times Log Saba price

* `loglacatanownpriceinter_std`: Lacatan banana production standard deviation times Log Lacatan price

* `logsugarinter_std`: Sugar production standard deviation times Log sugar price

* `logriceinter_std`: Rice production standard deviation times Log rice price  

* `noinfilXlogsugarinter_std`: Sugar production standard deviation times Log sugar price times low insurgent control

* `infil_pcntg0Xlogsugarinter_std`: Sugar production standard deviation times Log sugar price times medium insurgent influence

* `infil_pcntg50Xlogsugarinter_std`: Sugar production standard deviation times Log sugar price times high insurgent influence

* `infiltrated`: Number of villages controlled by the NPA   

* `cavendish_z`: Cavendish banana price expressed as standard deviation from the average Cavendish banana price over the observation period

* `lacatan_z`: Lacatan banana price expressed as standard deviation from the average Lacatan banana price over the observation period   

* `saba_z`: Saba banana price expressed as standard deviation from the average Saba banana price over the observation period

* `rice_z`: Rice price expressed as standard deviation from the average rice price over the observation period   

* `sugar_z`: Sugar price expressed as standard deviation from the average sugar price over the observation period    

* `plantationmedinter`: Cavendish banana production times Log price times plantation percentage below median       

* `plantationmedinter2`: Cavendish banana production times Log price times plantation percentage above median   

* `plantationtreesinter`: Banana trees in plantations (per capita) times Log price

* `bananatreesinter`: Banana trees (per capita) times Log price

* `roadinter`: Cavendish banana production times Log price times road density                    
* `musliminter`: Cavendish banana production times Log price times percentage of muslim population

* `povertyinter`: Cavendish banana production times Log price times poverty rate

#>

#< quiz "Data Set Knowledge"

question: Given the data set presented above, what information does one single row of observation convey?
sc:
    - One row records information for one specific year, in all provinces
    - One row records information for one specific year, in one specific province*
success: Great, you are correct! The data set consists of observations for different years (2001 to 2009) and provinces in the Philippines. One row of observation records several observed variables for one specific year, in one specific province of the Philippines. The provinces are identified by their latitude and longitude coordinates, as well as their unique province code (pcode). The several observed variables contain information about weather data, conflict incidents, prices, banana production volumes, rebel strength, and infrastructure data.

failure: Unfortunately, you messed up the fifty-fifty-chance, sorry.

#>

#< award "Rookie R Award"

**Congratulations!** This award indicates that you have successfully mastered the *R programming fundamentals*, such as inserting new data frames and basic data visualization. With your displayed passion and ent`h`usiasm, you are a promising contender for future tasks. *Let us continue to dive deeper into the challenging and beautiful world of R!* 
...
**To formulate the criterion, certain logical operators are available for usage:**
...
#>
summary_data_YES <- subset(data, meancavendish > 0)
...
summary_data_YES <- subset(data, meancavendish > 0) %>%
  select(cavendish2002_pop, lacatan2002_pop, saba2002_pop, sugar2002_pop, rice2002_pop, vinc, govtvinc, envinc, civvinc, govinitvinc, eninitvinc, ctm_vinc, mus_vinc, asg_vinc, le_vinc, totcas)
...
#>
mean(summary_data_YES$cavendish2002_pop)
#< hint
display("In case you don't remember the functions for the mean and standard deviation, just take a quick look above Task 8!")
#>
sd(summary_data_YES$cavendish2002_pop)
#< hint
display("In case you don't remember the functions for the mean and standard deviation, just take a quick look above Task 8!")
#>
...
#>
summary_data_NO <- subset(data, meancavendish == 0) %>%
  select(cavendish2002_pop, lacatan2002_pop, saba2002_pop, sugar2002_pop, rice2002_pop, vinc, govtvinc, envinc, civvinc, govinitvinc, eninitvinc, ctm_vinc, mus_vinc, asg_vinc, le_vinc, totcas)
...
The `kableExtra` package facilitates the creation of tables with *enhanced rendering ability*. Therefore, it enables the production of more *complex and aesthetically pleasing tables for HTML and Latex applications*. The package is very practical because it allows the modification of the appearance of the data frame in several layers of the table with the help of the Pipe Operator `%>%`.

Since the package offers a variety of customization options that exceed the scope of this Problem Set and are not discussed here. The [documentation](https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_html.html) of the package provides further information on its features and usage. Only the most important parameters and functions relevant for this Problem Set are described in the following list:

* `format`: Defines the rendering output format of the table
* `caption`: Defines the title of the table
* `position`: Aligns the table to the center, left or right side of the page
* `kable_styling()`: Gives the option to manually modify the style of the table
* `add_header_above()`: Inserts a new row with labels on top of the table
* `row_spec()`: Specifies the attributes of rows such as font, color, background, etc.
* `footnote()`: Adds footnotes with various specifications and notation systems
...
df_figure_rep <- read_dta("df_figure_rep.dta")
...
`ggplot2` is a system for creating graphics that *allows the user to specify the data, the mapping of variables to aesthetics and the graphical primitives to apply*. The function `ggplot()` initiates the plotting process by defining the data set and the aesthetic mapping with the function `aes()`. **This is the essential part of the graphic!**

```{r "2.2.8", eval= FALSE}

library("ggplot2")

ggplot(data = data_set, mapping = aes(x = values_x_axis, y = values_y_axis))

```

All further customizations can be appended to this foundation using the **+** operator. **These customizations include adding layers, scales, faceting specifications, or coordinate systems.** 

```{r "2.2.9", eval= FALSE}

ggplot(data = data_set, mapping = aes(x = values_x_axis, y = values_y_axis) + 
         geom_line(color = "red", linetype = "dashed", linewidth = 1.5) + #Layer as a line of data points with color and line specification
         scale_x_continuous(breaks = seq(4, 20, by = 2) + #The scale on the x-axis is specified and should go from 4 to 20 and this always in two steps
         labs(x = "x_axis_title", y = "y_axis_title", title = "plot_title") + #Defines the labs on both axis and the title of the plot

```

The function `theme()` can be used to modify the appearance of the plot elements.

```{r "2.2.10", eval= FALSE}

ggplot(data = data_set, mapping = aes(x = values_x_axis, y = values_y_axis)) + 
         geom_line(color = "red", linetype = "dashed", linewidth = 1.5) + 
         scale_x_continuous(breaks = seq(4, 20, by = 2)) + 
         labs(x = "x_axis_title", y = "y_axis_title", title = "plot_title") + 
         theme(
           panel.grid.major = element_line(color = "blue"), #Defines the major plot grid line color 
           plot.background = element_rect(fill = "yellow") #Defines the plot background color
           )

```

This introduction has demonstrated the diversity of `ggplot2` in creating graphics that can be customized to suit the users preferences and needs. It offers a wide range of graphical and visual adjustments that can *enhance the presentation and interpretation of the data*. The users can apply their creativity and expertise to produce individualized graphs without any limits. For further exploration of the topic, you can consult this [documentation](https://ggplot2.tidyverse.org/reference/), which provides a comprehensive overview of all the options available with `ggplot2`. 
...
```{r "2.2.11", fig.width = 10, fig.height = 6}
...
Figure_2_banana
#>
Figure_2_banana <- ggplot(data = df_figure_rep, aes(x = year, y = bananaprice)) + #Save this plot in the variable "Figure_2_banana". The data are from the data frame "df_figure_rep". In the mapping, for the aesthetics, we display the column "year" on the x-axis and "bananaprice" on the y-axis
  geom_line(color = "blue", linetype = "solid", linewidth = 1.5) + #The illustrated line for the "bananaprice" should be blue and solid with a linewidth of 1.5 
  ...
  labs(x = "Year", y = "Price (2010 $/kg)", title = "Cavendish Banana") + #Here we define our labs on both axes and the title of this plot
  theme_bw() + #Defines the plot theme
  theme(
    plot.title = element_text(hjust = 0.5),  #Defines the position of the title.
    panel.grid.major = element_line(color = "gray"), #Sets the major plot grid line color to grey
    panel.grid.minor = element_blank(), #Sets the minor plot grid line color to blank.
    axis.text = element_text(color = "black"), #Sets the axis text color to black.
    axis.ticks = element_line(color = "black"), #Sets the axis ticks line color to black
    plot.background = element_rect(fill = "white"), #Sets the plot background color to white
    panel.background = element_rect(fill = "white") #Sets the panel background color to white
  )
...
```{r "2.2.12", fig.width = 10, fig.height = 6}
...
#>
#Creates the plot according to the same principle as for "Figure_2_banana" above, only with the "sugarprice" on the y-axis
Figure_2_sugar <- ggplot(data = df_figure_rep, aes(x = year, y = sugarprice)) +
  geom_line(color = "blue", linetype = "solid", linewidth = 1.5) +
  scale_x_continuous(breaks = seq(2000, 2010, by = 2), limits = c(2000, 2010)) +
  labs(x = "Year", y = "Price (2010 $/kg)", title = "Sugar") +
  theme_bw() +
...
#Creates the plot according to the same principle as for "Figure_2_banana" above, only with the "lacatanprice" on the y-axis
Figure_2_lacatan <- ggplot(data = df_figure_rep, aes(x = year, y = lacatanprice)) +
  geom_line(color = "blue", linetype = "solid", linewidth = 1.5) +
  scale_x_continuous(breaks = seq(2000, 2010, by = 2), limits = c(2000, 2010)) +
  labs(x = "Year", y = "Price (2010 $/kg)", title = "Lacatan Banana") +
  theme_bw() +
  theme(
    plot.title = element_text(hjust = 0.5),
    panel.grid.major = element_line(color = "gray"),
...
#Creates the plot according to the same principle as for "Figure_2_banana" above, only with the "sabaprice" on the y-axis
Figure_2_saba <- ggplot(data = df_figure_rep, aes(x = year, y = sabaprice)) +
  geom_line(color = "blue", linetype = "solid", linewidth = 1.5) +
  scale_x_continuous(breaks = seq(2000, 2010, by = 2), limits = c(2000, 2010)) +
  labs(x = "Year", y = "Price (2010 $/kg)", title = "Saba Banana") +
  theme_bw() +
...
#We want to illustrate our 4 created plots in one figure. We do so with the "ggarrange()" function from the "ggpubr" package and specify the number of columns and rows in the figure
library("ggpubr")

Figure_2 <- ggarrange(Figure_2_banana, Figure_2_sugar, Figure_2_lacatan, Figure_2_saba, ncol = 2, nrow = 2)

Figure_2 #Shows the created figure
...
#< quiz "Linearity of Cavendish banana Price movement"

question: If you look at the price development of the Cavendish banana variety, how would you categorize the movement?
sc:
    - As a linear function
    - As a nonlinear function*
success: Great, you are correct! Crost and Felter (2020, p. 1498) stated that the observed nonlinear Cavendish banana price function enables us to recognize the effect of Cavendish value fluctuations, with control for unknown variables that vary linearly across Cavendish manufacturing and non-producing provinces. The identification is less robust for sugar price movements, which mostly exhibit an upward trend, except for the sharp drop in 2007. It is even weaker for Lacatan and Saba bananas, whose price movements are hard to differentiate from a linear trend. Given these constraints, we will therefore focus our analysis on Cavendish bananas!

failure: Unfortunately, you messed up the fifty-fifty-chance, sorry.

#>

#< award "GGPLOT Rookie Medal"

**Congratulations!** You have demonstrated the ability to use R functions, operators, and data structures to perform various tasks on the data frame. Moreover, you have examined the core descriptive statistics within our data set and analyzed the implications arising from fluctuations in commodity prices. 

These findings will serve as a foundation framework for our further investigations.

You have also unleashed your creativi`t`y and produced your first own stunning visualization using the powerful `ggplot2` tool. You have proven to be a fast learner and a skilled person and showed the potential to apply R to more advanced topics. *You have mastered the basics of layers and themes and created plots that look like dreams!* 

**Excellent!** You are doing a fantastic job of filling your trophy cabinet. Keep pushing and you will become a legend!
...
Using the main `mutate()` function, provided by the `dplyr` package, allows fundamental changes to data frame architectures. This facilitates the creation, modification, and elimination of designated columns within a data frame. In addition, the function can also be applied to a tibble.  

In particular, the function is also able to execute actions within each group while maintaining the integrity of the data frame structure. This feature makes the function an exceptionally powerful tool for manipulating grouped data sets.

Overall, the `mutate()` function is a high performance tool for R data manipulation.  

A nicely illustrated documentation of the `mutate()` function can be accessed [here](https://dplyr.tidyverse.org/reference/mutate.html).
...
#>
y_pre_tr <- mean(filter(data_DiD, treat == 1, exp == 0)$vinc) %>% round(3)
y_exp_tr <- mean(filter(data_DiD, treat == 1, exp == 1)$vinc) %>% round(3)
y_pre_co <- mean(filter(data_DiD, treat == 0, exp == 0)$vinc) %>% round(3)
y_exp_co <- mean(filter(data_DiD, treat == 0, exp == 1)$vinc) %>% round(3)
...
#>
y_exp <- y_exp_tr - y_exp_co

y_pre <- y_pre_tr - y_pre_co
...
The `group_by()` and `summarise()` functions from the `dplyr` package allow us to perform aggregation operations on a data frame based on a grouping variable. For example, suppose we have a data frame that contains the year-to-date returns of various companies across different industries for the past 10 years. If we want to calculate the average return for each industry over this period, we can use the following steps. 

First, we use the `group_by()` function to create a grouped data frame, where each group consists of the rows that share the same value for the industry variable. 

Second, we use the `summarise()` function to apply a mathematical function to each group, resulting in a new data frame with one row per group and one column for the mathematical value. In this case, the mathematical function is `mean()`, which computes the *mean year-to-date industry return*.
...
return_data %>%
  group_by(industry_variable) %>%
  summarise(Industry mean = mean()) #Industry mean is the name of the newly created column

```

To gain further insight into the [group_by()](https://dplyr.tidyverse.org/reference/group_by.html) and [summarise()](https://dplyr.tidyverse.org/reference/summarise.html) functions, consult their respective documentation.
...
```{r "3.1.8", fig.width = 10, fig.height = 6}
#< fill_in
library("ggplot2")
...
data_table %>% 
  group_by(pcode) %>% 
  summarise("Number_of_reported_years" = n()) %>% 
  ggplot(data = ., mapping = aes(x = Number_of_reported_years)) + 
...
reg_DiD = feols(vinc ~ treat_exp | year + pcode, data = data_DiD)

modelsummary(reg_DiD,
             title = "Verifying the DiD coefficient of the initial small analysis", 
...
question: We end this exercise with a small but **mandatory Quiz**. Is the obtained positive DiD treatment effect in line with the knowledge from Exercise 2 about the CSF and the mechanisms of the relationship between varying prices of Cavendish bananas and conflict violence? Does it make sense intuitively?
sc:
    - It is consistent with the knowledge gained*
    - It is inconsistent with the knowledge gained
success: Great, you are correct! In Exercise 2, we acquired the theoretical understanding that a rise in the global market price of Cavendish bananas induces a relatively higher frequency of violent incidents in provinces with Cavendish cultivation. This is occupied by the positive DiD estimator, which is what we would intuitively expect.

failure: Try again.
#>

#< award "DiD specialist"

**Congratulations!** You have acquired a comprehensive understanding of the DiD methodology. Your familiarity extends to the core investigative framework utilized within the study. Moreover, you possess the capability to compute the DiD estimator both manually and through regression ana`l`ysis. Additionally, you have a thorough comprehension of the concept of Fixed Effects within the context of the DiD setting.

The preliminary phase of iterative processing, which forms the basis of the comprehensive analysis presented in the closing section of Chapter 3, has been completed. Subsequent exercises will involve the implementation of changes based on this established foundation. **Keep up the good work!**
#>
...
DiD_reg_iteration = feols(vinc ~ treat_price | year + pcode, data = data_DiD_modify)

modelsummary(DiD_reg_iteration,
             title = "DiD Iteration Step 1 towards the Key Regression Analysis", 
             stars = c("*" = 0.1, "**" = 0.05, "***" = 0.01), 
             fmt = 3, 
             gof_omit = "R2 Within Adj.|AIC|BIC|Std.Errors|FE:", 
...
#>
mean_cavendish_price <- mean(data_DiD_modify$cavendish_price)
#< hint
display("Use the appropriate mean function. Remember the correct price variable we want to apply it to!")
#>
...
```{r "3.2.7", fig.width = 10, fig.height = 8}
...
#>
DiD_reg_iteration_1 = feols(vinc ~ treat_price + zrain + wetrain + wettemp + temperature + typhoon | year + pcode, data = data_DiD_modify)
#< hint
display("Use the appropriate feols() function! Include all Control Variables in the order you obtained above!")
#>

modelsummary(DiD_reg_iteration_1,
             title = "DiD Iteration Step 1 with Control Variables", 
             stars = c("*" = 0.1, "**" = 0.05, "***" = 0.01), 
             fmt = 3, 
             gof_omit = "R2 Within Adj.|AIC|BIC|Std.Errors|FE:", 
...
dat1 <- data_DiD_modify %>% 
  mutate(
   cavendish_price = mean_cavendish_price,
   zrain_mean = mean(zrain),
   wetrain_mean = mean(wetrain),
   wettemp_mean = mean(wettemp),
   temperature_mean = mean(temperature),
   typhoon_mean = mean(typhoon),
   treat_price = treat * cavendish_price
)
```
...
```{r "3.2.11", fig.width = 10, fig.height = 8}
...
#< quiz "Control Variables Conclusion"

question: Through the findings of the graphical representation above, does it make sense to incorporate Control Variables into ongoing regression analyses?
sc:
    - It does make sense*
    - It does not make sense
success: Great, you are correct! Through the representation above, we find strong graphical evidence that it makes sense to fully incorporate Control Variables into ongoing regression analyses to account for specific weather conditions that have an impact on the production of Cavendish bananas and thus on conflict! This is confirmed by the pre-treatment trend, which is more parallel than without Control Variables.

failure: Try again.
#>

#< award "Master of Parallel Trends"

**Congratulations!** Through this exercise, you have gained an understanding of the process of *refining the preliminary regression analysis to bring the main regression into focus.* You have also gained an understanding of the central price variable and its implications for regional conflict dynamics.

Furthermore, you have acquired a deeper understanding of the methodologies employed to validate the parallel `t`rend assumption embedded in the DiD framework.

In addition, you have become familiarized with the concept of Control Variables, the specific challenges they address and the appropriate methodology for integrating them into subsequent analytical processes. **Great job!** 

**Unfortunately, we are not done yet! One iteration step is still missing to get to the key regression analysis of the study, employed by the authors!**
#>
...
#< quiz "Last Iteration Process"

question: Throughout the iterative process, we have used the interaction term `treat_price` as a way of explaining the provincial incidence of violence. Consequently, what modifications can be made to this interaction term in the final iteration process to capture the one of the key analysis results?
sc:
    - Integrate the provincial code
    - Integrate the provincial Cavendish production*
    - Integrate the provincial weather conditions
success: Great, you are correct! It is essential to integrate the Cavendish production data into the regression analysis framework! This requires replacing the treatment component within the interaction term, with the output figures for Cavendish production at provincial level.

failure: Try again. Take a closer look at the interaction term of the key analysis result!
...
library("haven")
#< hint
display("In case you don't remember how to load a package, just take a quick look at the Info block at the start of Exercise 2.1!")
#>

data <- read_dta("replication_data.dta")
#< hint
display("In case you don't remember how to read in a data frame, just take a quick look at the Info block at the start of Exercise 2.1! Remember the quotation marks around the file name!")
#>
...
df_modify_CV <- data_table %>%
  mutate(
    ...
    cavendish_production = cavendish2002_pop,
    group = ifelse(cavendish2002_pop == 0, "control", "treat"),
    prod_price = cavendish_production * cavendish_price
  )
```
...
DiD_reg_CV = feols(vinc ~ prod_price + zrain + wetrain + wettemp + temperature + typhoon | year + pcode, data = df_modify_CV)

modelsummary(DiD_reg_CV,
             title = "Last Iteration Step with Cavendish Banana Value as Regressor", 
             stars = c("*" = 0.1, "**" = 0.05, "***" = 0.01), 
             fmt = 3, 
             gof_omit = "R2 Within Adj.|AIC|BIC|Std.Errors|FE:", 
...
#< quiz "Robustness Check and Null Hypothesis"

question: What do the insignificant and negligible lead coefficients imply?
sc:
    - The null hypothesis can be rejected
    - The null hypothesis cannot be rejected*
success: Great, you are correct! The lead coefficients imply that the null hypothesis cannot be rejected. This strengthens the credibility of the parallel trend assumption. Thus, Crost and Felter (2020, p. 1500) can infer the causal impact of the crop value shock on the conflict outcomes. It also demonstrates that the future world market price has no effect on the actual conflict intensity in the Cavendish-producing provinces.

failure: Try again! Review the statement of the null hypothesis.

#>

#< award "Analyst"

You have acquired a solid understanding of the regression-based DiD method, which incorporates FE, Control Variables, and Cluster Robust Standard Errors. You have also gained insights into the papers baseline regression appr`o`ach and conducted your own regression analysis. Moreover, you have earned the ability to interpret the results of the regression analysis presented in a tabular format. 

**Congratulations on your excellent progress and keep up the good work!**
...
merged_data <- read_dta("merged_data.dta")
...
```{r "4.1.2", fig.width = 10, fig.height = 6}
#< fill_in
library("ggplot2")
library("ggpubr")
...
Figure_3_Bananas <- ggplot(data = merged_data, aes(x = year)) +  
  geom_line(aes(y = beta, color = "slope coefficient (θ)"), linetype = "solid", linewidth = 1.5) + 
  geom_line(aes(y = a_1 + b_1 * bananaprice, color = "Cavendish price"), linetype = "dashed", linewidth = 1.5) + 
  scale_y_continuous(name = "slope coefficient (θ)", sec.axis = sec_axis(~ (. - a_1) / b_1, name = "price (2010 $/kg)")) + 
  scale_color_manual(values = c("blue", "red")) + 
  scale_x_continuous(breaks = seq(2000, 2010, by = 2), limits = c(2000, 2010)) + 
...
Figure_3_Conf <- ggplot(data = merged_data, aes(x = year)) +
  geom_ribbon(aes(ymin = cilow, ymax = cihigh, fill = "confidence interval"), alpha = 0.5) + 
  geom_line(aes(y = beta, color = "slope coefficient (θ)"), linetype = "solid", linewidth = 1.5) +
  geom_line(aes(y = a_2 + b_2 * bananaprice, color = "Cavendish price"), linetype = "dashed", linewidth = 1.5) + 
  scale_y_continuous(name = "slope coefficient (θ)", sec.axis = sec_axis(~ (. - a_2) / b_2, breaks = seq(0, 1.6, by = 0.4), name = "price (2010 $/kg)")) +
  scale_color_manual(values = c("blue", "red")) +
...
#< quiz "IV Validity Check I"

question: In accordance with the assumptions previously studied, do you consider the IV in scenario a) of Figure 12 above to be valid?
sc:
    - Valid Instrumental Variable
    - Invalid Instrumental Variable*
success: Great, you are correct! The instrument $Z$ must satisfy the exogeneity restriction. However, in scenario a), $Z$ violates this condition because it is related to $U$. This creates a spurious correlation between $Z$ and the outcome variable, distorting the causal inference made by the Cavendish value on the number of provincial violent incidents. Therefore, we can reject that $Z$ is a valid instrument, see Pokropek (2016, p.4) for more details. 

failure: Try again! Review the necessary assumptions for a valid IV.

#>

#< quiz "IV Validity Check II"

question: In accordance with the assumptions previously studied, do you consider the IV in scenario b) of Figure 12 above to be valid?
sc:
    - Valid Instrumental Variable
    - Invalid Instrumental Variable*
success: Great, you are correct! $Z$ is not a valid instrument in scenario b) because it does not satisfy the exclusion restriction. As learned by Pokropek (2016, p.4), this requires that $Z$ only impacts the outcome variable through the Cavendish value. However, $Z$ is a regressor in that model, which means that it has a direct influence on the number of violent disputes.

failure: Try again! Review the necessary assumptions for a valid IV.

#>

#< quiz "IV Validity Check III"

question: In accordance with the assumptions previously studied, do you consider the IV in scenario c) of Figure 12 above to be valid?
sc:
    - Valid Instrumental Variable*
    - Invalid Instrumental Variable
success: Great, you are correct! A valid instrument for the endogenous regressor of Cavendish value is found in the scenario c). In that, $Z$ has no connection to the unobservable factors $U$. Moreover, $Z$ is no independent variable in the regression on the outcome variable. Therefore, $Z$ has no direct influence on the provincial incidents, but only through the Cavendish banana value. According to Pokropek (2016, p.4), all necessary conditions for a valid instrument are matched here. 
...
The regression analysis with *IV, FE, Control Variables, and Cluster Robust Standard Errors* is performed using the central `feols()` function that you have previously learned. Recall the syntax of the function:

**feols(dependent variable ~ independent variables + Control Variables | Fixed Effects | Instrumental Variable, data, weights, cluster)**

Following the second vertical line, an *IV argument can be incorporated into the approach*. 

This requires specifying the argument as: **endogenous independent variable ~ IV**. The `feols` syntax remains unchanged for the rest of the model.
...
Table_3_list <- list(
  ...
  "(1)" = feols(vinc ~ temperature + typhoon | year + pcode | logcavendishinter2002_pop ~ logpinter_pop, data = data_table, weights =  data_table$pop2000, cluster = data_table$pcode), 
   
  "(2)" = feols(vinc ~ temperature + typhoon + zrain + wetrain | year + pcode | logcavendishinter2002_pop ~ logpinter_pop, data = data_table, weights =  data_table$pop2000, cluster = data_table$pcode),
  
  "(3)" = feols(logcavendishinter2002_pop ~ logpinter_pop | year + pcode, data = data_table, weights =  data_table$pop2000, cluster = data_table$pcode),
  
  "(4)" = feols(logcavendishinter2002_pop ~ logpinter_pop + zrain + wetrain + temperature + typhoon | year + pcode, data = data_table, weights = data_table$pop2000, cluster = data_table$pcode)
...
#< quiz "Table 3 Implications"

question: Given the magnitude of the coefficients in columns (1) and (2) of Table 3, does the positive relationship between elevated levels of rainfall in Ecuador and the global banana price encourage the occurrence of reverse causality?
sc:
    - It does support the presence of reverse causality
    - It does not support the presence of reverse causality*
success: Great, you are correct! This positive relationship in combination with the magnitude of the estimated coefficients, which are similar to those of Table 2, denies the possibility of reverse causality affecting the coefficients. It implies that the possible endogeneity of the market price did not bias the previous estimates in Table 2!

failure: Try again!

#>

#< award "Honor for Conductors"

**Great!** Your understanding of the modifications made to the basic regression procedure and the importance of the central theta coefficient is to be appreciated.

Furthermore, you have acquired the knowledge that the concept of **instrument** extends beyond its musical meaning to denote a methodological tool in `r`egression analysis. We used it to investigate potential issues of reverse causality and to support the overall findings presented in Table 2.

**Excellent progress!**

In the next exercise we shed light on the concept of spatial regressions to account for provincial geographic correlation.
#>
...
The `conleyreg()` function consists of several components that are essential for our analysis and are explained in this block. For a comprehensive overview of the syntax and the functionality of the arguments, please refer to the documentation at the link [here](https://rdrr.io/cran/conleyreg/man/conleyreg.html). 

For our purposes, the syntax is as follows: 

*conleyreg(formula, data, dist_cutoff, unit, time, lat, lon, lag_cutoff)*

* `formula`: The regression model that we would like to estimate

* `data`: The set of data on which the regression analysis is to be applied, with geo-based information

* `dist_cutoff`: Defines spatial bandwidth (in km)

* `unit`: Indicates the stationary variable that does not shift its position as time goes on

* `time`: Indicates the time variable

* `lat`: Determines the latitudinal geographic coordinates

* `lon`: Determines the longitudinal geographic coordinates

* `lag_cutoff`: Defines the maximum lag of the time variable
...
#>
data_table_mind_visa <- data_table %>%
  filter(., mindanao == 1 | visayas == 1) 
...
#>
data_table_mindanao <- data_table_mind_visa %>%
  filter(., mindanao == 1)
...
Table_4_list <- list(
  ...
  "(1)" = feols(vinc ~ logcavendishinter2002_pop + zrain + wetrain + wettemp + temperature + typhoon | year + pcode + mindanao * year + visayas * year, data = data_table, weights =  data_table$pop2000, cluster = data_table$pcode),
  
  "(2)" = feols(vinc ~ logcavendishinter2002_pop + zrain + wetrain + wettemp + temperature + typhoon | year + pcode + mindanao * year, data = data_table_mind_visa, weights =  data_table_mind_visa$pop2000, cluster = data_table_mind_visa$pcode),
  
  "(3)" = feols(vinc ~ logcavendishinter2002_pop + zrain + wetrain + wettemp + temperature + typhoon | year + pcode, data = data_table_mindanao, weights =  data_table_mindanao$pop2000, cluster = data_table_mindanao$pcode),

  "(4)" = conleyreg(vinc ~ logcavendishinter2002_pop + zrain + wetrain + wettemp + temperature + typhoon + as.factor(pcode) + as.factor(year), data = data_table, dist_cutoff = 5000, lat = "latcoord", lon = "longcoord", time = "year", unit = "pcode", lag_cutoff = 9) 
...
#< quiz "Evidence Table 4"

question: How does the evidence from Table 4 affect our confidence in the unbiasedness of the baseline regression estimates in Table 2, concerning territorial correlation?
sc:
    - More confident*
    - Less confident
success: Great, you are correct! As this is proven by the table and mentioned by Crost and Felter (2020, p. 1502), we are now more confident that the basic regressions in Table 2 have unbiased coefficients and standard errors, because they are not affected by hidden geographically correlated impacts in areas where a lot of Cavendish bananas are produced for export.

failure: Try again!

#>

#< award "Territorial Medal"

**Great!** You have performed an in-depth analysis of the basic regressions from Table 2 and tested their robustness using various statistical methods. You have examined the concept of *spatial regression and the Conley approach* to deal with spatial autocorrelation. **Well done!**

Next, we will conduct further analyses of the relationship between Cavendish banana production, price, and rebel violence in the Philippin`e`s. In particular, we analyze the various rebel organizations and the insurgent control in the nation.
...
#< quiz "Hypothesis Table 5"

question: Based on the analytical outcomes presented in Table 5, is it possible to reject the hypothesis by Crost and Felter (2020, p. 1504) regarding the strength of the rebel organizations?
sc:
    - The hypothesis can be rejected
    - The hypothesis cannot be rejected*
success: Great, you are correct! The table provides evidence that we cannot reject the hypothesis that a higher Cavendish banana value enhance the strength of the rebel organizations to effectively confront any opposing parties. The coefficient for the insurgent-initiated violent accidents is statistically significant at an elevated level with a large mean. Furthermore, the average number of armed conflicts that have resulted in at least one fatality is significantly greater for the governmental side, suggesting that the overall insurgent strength escalates with higher banana values!

failure: Try again! Review Table 5 and the hypothesis again.
...
When dealing with data frames that contain missing values for a certain variable, we can apply the `complete.cases()` function from the standard R, to filter out the rows that have NA values in the corresponding column.

The following is a short example of how to use the function to achieve that goal: 

Consider a scenario wherein a *monitoring device records the temperature and humidity levels within a room at one-minute intervals*, storing the acquired data in a file. Unfortunately, a temporary *power outage* disrupts the device's operation for two minutes, after which normal functionality is restored. Consequently, the data file manifests two instances of missing values, represented as NAs, corresponding to the temperature in degrees and humidity as a percentage of the maximum water vapor capacity in the air at a given temperature. Additionally, data relating to the *presence of the occupants in the room* is incorporated into the existing data frame as an independent column, irrespective of the occurrence of the power outage.

The following data frame in R illustrates this example, if the shortage occurred within a five-minute period: 

```{r "5.1.5", eval= FALSE}

measure_data <- data.frame(
                        "temp" = c(22, 21, NA, NA, 22), 
                        "humid" = c(40, 43, NA, NA, 45),
                        "persons" = c(5, 5, 5, 4, 4))
...
The analysis aims to *calculate the mean of the temperature variable*. However, due to the presence of missing values, this is *not feasible*. Therefore, the *NAs need to be excluded from this specific column*. To achieve this, the `complete.cases()` function is applied. **This function returns TRUE for values and FALSE for NAs, of the selected column in the data frame**.

```{r "5.1.6", eval= FALSE}

complete.cases(measure_data$temp)
...
One FALSE value indicates the presence of a NA. To eliminate these rows, which have NAs for the temperature column, the original data frame can be filtered by **selecting only the rows that have values for the temperature record (TRUE)**. Therefore, we can specifically *adjust the rows and columns of a data frame* with **square brackets!** On the *left of the comma, we can modify the rows, and on the right the columns!* 

```{r "5.1.7", eval= FALSE}

measure_data_complete <- measure_data[complete.cases(measure_data$temp), ] 

With the [] we therefore just have left the rows, for which we have recorded temperature values (TRUE), and that for all columns of the original data frame.
...
The temperature column has been cleared of NA values, enabling the calculation of the mean. *Rows 3 and 4 have been excluded from all columns!*
...
#>
data_table_complete <- data_table[complete.cases(data_table$povertyinter), ]
...
Table_7_list <- list(
  ...
  "(1)" = feols(vinc ~ logcavendishinter2002_pop + povertyinter + zrain + wetrain + wettemp + temperature + typhoon | year + pcode, data = data_table_complete, weights =  data_table_complete$pop2000, cluster = data_table_complete$pcode),
  
  "(2)" = feols(vinc ~ logcavendishinter2002_pop + musliminter + zrain + wetrain + wettemp + temperature + typhoon | year + pcode, data = data_table, weights =  data_table$pop2000, cluster = data_table$pcode),
  
  "(3)" = feols(vinc ~ logcavendishinter2002_pop + roadinter + zrain + wetrain + wettemp + temperature + typhoon | year + pcode, data = data_table, weights =  data_table$pop2000, cluster = data_table$pcode),
  
  "(4)" = feols(vinc ~ logcavendishinter2002_pop + povertyinter + roadinter + musliminter + zrain + wetrain + wettemp + temperature + typhoon | year + pcode, data = data_table_complete, weights =  data_table_complete$pop2000, cluster = data_table_complete$pcode)
...
#< quiz "Intention Table 7"

question: How does this result reflect the author's intention for considering the factor of road infrastructure in their analysis?
sc:
    - It is in line with the intention of the authors*
    - It is not in line with the intention of the authors
success: Great, you are correct! As previously articulated by Crost and Felter (2020, p. 1505), a plausible rationale is that an augmented road system facilitates accelerated governmental reactions to extortion trials, thereby enabling the assertive establishment of territorial control, within the specific province.

failure: Unfortunately, you messed up the fifty-fifty-chance, sorry.

#>

#< award "Insurgent Impact Award"

**Congratulations!** In this exercise, you have conducted further ana`l`yses of the impact of insurgent violence and its possible heterogeneous effects. You have gained an understanding of the *initiators and the victims of violence* in the Philippines during the observation period, as well as the *main insurgent group involved in it*. Additionally, you have explored some of the *factors that may influence the number of violent incidents at the provincial level*, using a novel R function. **Well done!**

In the next chapter, we will come up with some mechanisms for the interpretation of the findings!
...
#>
data_table_inf <- data_table[complete.cases(data_table$infiltrated), ]
...
Table_9_list <- list(
  ...
  "(1)" = feols(vinc ~ plantationmedinter + plantationmedinter2 | year + pcode, data = data_table, weights =  data_table$pop2000, cluster = data_table$pcode),
  
  "(2)" = feols(vinc ~ plantationmedinter + plantationmedinter2 + zrain + wetrain + wettemp + temperature + typhoon | year + pcode, data = data_table, weights =  data_table$pop2000, cluster = data_table$pcode),
  
  "(3)" = feols(vinc ~ plantationtreesinter + bananatreesinter | year + pcode, data = data_table, weights =  data_table$pop2000, cluster = data_table$pcode),
  
  "(4)" = feols(vinc ~ plantationtreesinter + bananatreesinter + zrain + wetrain + wettemp + temperature + typhoon | year + pcode, data = data_table, weights =  data_table$pop2000, cluster = data_table$pcode)
...
#< quiz "Commodities Table 10"

question: Based on the estimates presented in Table 10, which commodities are statistically not meaningful in explaining the variation of civil disputes?
sc:
    - Cavendish banana, Saba banana and Lacatan banana
    - Cavendish banana, rice, and sugar
    - Rice, Lacatan banana and Saba banana*
    - Sugar, rice, and Saba banana
success: Great, you are correct! Like Crost and Felter (2020, p. 1509), we do not detect any statistically meaningful impacts of changing rice values on armed disputes. This is also applicable for the local based Lacatan and Saba banana varieties. Interestingly, the domestic markets, on which these commodities are all traded, are strongly fragmented!

failure: Try again! Take another close look at Table 10.
...
```{r "6.1.8", fig.width = 10, fig.height = 10}
...
#< quiz "Commodity Hypothesis Figure 4"

question: Based on the graphical analysis presented in this figure, how do these results compare with the tabular findings reported in Table 10?
sc:
    - The results are consistent with the findings of Table 10*
    - The results are inconsistent with the findings of Table 10
success: Great, you are correct! As shown in Table 10, the number of violent disputes is significantly affected only by the commodities of Cavendish banana and sugar, which are the main Philippine export goods that are traded on highly concentrated markets. This is consistent with the outcomes of the displayed figure and Crost and Felter (2020, p. 1509), which indicate that during the monitoring period, the theta coefficients of the Cavendish banana and sugar crop carefully track their respective price developments. However, for the remaining three plants, the theta coefficients and the price tendencies are more differentiated. Thus, we can validate the findings of Table 10 with this graphical analysis!

failure: Try again! Take another close look at the figure and the outcomes for the Cavendish banana and sugar commodity. 
...
#< quiz "Transaction Costs of Blackmailing"

question: Given this premise, what are the implications of observing high transaction costs?
sc:
    - Elevated transaction costs may increase the number of blackmail attempts
    - Elevated transaction costs may decrease the number of blackmail attempts*
success: Great, you are correct! Like Crost and Felter (2020, p. 1511) mentioned, when transaction costs of engaging in armed disputes are excessively large, the incentives for extracting financial resources from these decentralized markets, as a source of income, diminish. This factor reduces the likelihood that the non-exporting goods influence provincial violence. 

failure: Try again! Think about the effect chain one more time. 

#>

#< award "Crop and Conflict Medal"

**Great!** By diving into the context of the Philippines and the impact of various commodities, particularly the Cavendish banana variety, on the prevalence of provincial violence, **you have successfully completed this Problem Set!** 

Moreover, you have investigate`d` the role of insurgent provincial control on violence with interesting insights about the NPA. Additionally, you have enlightened the findings with plausible mechanisms and supported them with appropriate regression analyses. **This award is a recognition of your persistence and excellence, congratulations!**

The final exercise will provide you with a summary of the Problem Set and a conclusion.
...
#>
SOLUTION_WORD <- c("TITLEHOLDER")
#< hint
display("The word we are looking for is someone who currently holds a record, especially in a competitive field such as sport or royalty!")
#>
...
Additionally, it is possible to export the created data frames in a `.dta` format, *that it can always be loaded without running the data preparation process!* To do so, make use of the `foreign` package in combination with the `write.dta()` function. In that, specify the selected data frame for export purpose, and the created file name with `.dta` ending. The data file will be stored in your current working directory:

```{r "A.2.21", eval=FALSE}

library("foreign")

write.dta(df_figure_rep, file = "df_figure_rep.dta")

write.dta(merged_data, file = "merged_data.dta")
